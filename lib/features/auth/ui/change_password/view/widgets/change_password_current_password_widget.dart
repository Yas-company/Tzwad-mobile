...
import 'package:tzwad_mobile/features/auth/ui/change_password/hooks/change_password_current_password_hook.dart';
import 'package:tzwad_mobile/features/auth/ui/change_password/providers/change_password_controller_provider.dart';

class ChangePasswordCurrentPasswordWidget extends HookConsumerWidget {
  const ChangePasswordCurrentPasswordWidget({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final errorText = ref.watch(
      changePasswordControllerProvider.select(
        (value) => value.currentPasswordValidationMessage,
      ),
    );
    final obscureText = ref.watch(
      changePasswordControllerProvider.select(
        (value) => value.obscureTextCurrentPassword,
      ),
    );
    final controller = useChangePasswordCurrentPasswordController(
      ref: ref,
      initialText: '',
    );
    return Column(
      ...
      children: [
        ...
        AppTextFieldWidget(
          ...
          suffixIcon: AppRippleWidget(
            radius: AppSize.s12,
            onTap: () {
              ref.read(changePasswordControllerProvider.notifier).changeVisibilityCurrentPassword();
            },
            child: Icon(
